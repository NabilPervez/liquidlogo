...
function updateUniforms() {
  ...
  glRef.value.uniform1f(uniforms.value.u_edge ?? null, props.edge);
  glRef.value.uniform1f(uniforms.value.u_patternBlur ?? null, props.patternBlur);
  glRef.value.uniform1f(uniforms.value.u_time ?? null, 0);
  glRef.value.uniform1f(uniforms.value.u_patternScale ?? null, props.patternScale);
  glRef.value.uniform1f(uniforms.value.u_refraction ?? null, props.refraction);
  glRef.value.uniform1f(uniforms.value.u_liquid ?? null, props.liquid);
}
...
function render(currentTime: number) {
  ...
  glRef.value!.uniform1f(uniforms.value.u_time ?? null, totalAnimationTime.value);
  // Draw!
...
async function cleanTexture() {
  ...
  try {
    ...
    gl.uniform1i(uniforms.value.u_image_texture ?? null, 0);
  } catch (e) {
    // handle error
  }
...
<template>
  <div
    ...
    class="text-primary/50 flex size-full items-center justify-center text-2xl font-bold"
  >
